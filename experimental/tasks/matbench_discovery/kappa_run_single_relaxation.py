...
def evaluate_tednet_kappa(path):
    ...
    with open(output_path, 'a') as file:
        file.write('kappa_srme for each structure: {}\n'.format(kappa_srme_list))
        file.write('kappa_srme: \t{}\n'.format(kappa_srme))
        file.write('kappa_sre: \t{}\n'.format(kappa_sre))
    print('kappa_srme for each structure: {}'.format(kappa_srme_list))
    print('kappa_srme: \t{}'.format(kappa_srme))
    print('kappa_sre: \t{}'.format(kappa_sre))

    

def kappa_run_relaxation(
    checkpoint_path: Annotated[Path, typer.Option()],
    output_dir: Annotated[Path, typer.Option(help="Output path to write results files")],
) -> None:
