...
class RelaxJob(Checkpointable):
    ...
    def __call__(
        ...
        run_name = 'index@{}-total@{}'.format(self.job_index, self.num_jobs)
    ...
    def _ase_relax(
        ...
        for idx in tqdm.tqdm(
            ...
            desc='job_index {}'.format(self.job_index),
        ):
            ...
            try:
                ...
                output_err_filename = "error_index@{}_total@{}.txt".format(
                    self.job_index,
                    self.num_jobs
                )
...
def run_relax(
    ...
    output_path: Annotated[Path, typer.Option(help="Output path to write results files")],
    optimizer: Annotated[
    ...
    use_amp: Annotated[bool, typer.Option(help="Use automatic mixed precision")] = False,
    num_jobs: Annotated[
    ...
    debug: Annotated[bool, typer.Option(help="Debug mode and evaluate on a subset of structures")] = False,
) -> None:
    setup_logging()  
